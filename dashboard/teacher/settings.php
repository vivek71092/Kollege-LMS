...
// Role-specific check
require_role(['teacher']); // Or adjust roles if this page is shared
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['change_password'])) {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
    ...
    } elseif (strlen($new_password) < 8) { // Basic length check (consider adding complexity check from utils/validators.php if needed)
        ...
        try {
            ...
            $current_hashed_password = $stmt_fetch->fetchColumn(); // Get only the password column
            ...
            if (password_verify($current_password, $current_hashed_password)) {
                ...
                if ($success) {
                    ...
                    // Logger::info("User ID {$user_id} changed their password.");
            ...
            log_error("Database error changing password for user ID {$user_id}: " . $e->getMessage(), __FILE__, __LINE__);
            $_SESSION['error_message'] = "A database error occurred while changing the password.";
        } catch (Exception $e) { // Catch other potential errors
             log_error("Error changing password for user ID {$user_id}: " . $e->getMessage(), __FILE__, __LINE__);
    ...
    redirect('dashboard/' . $user['role'] . '/settings.php');
}
// --- End POST Processing ---
